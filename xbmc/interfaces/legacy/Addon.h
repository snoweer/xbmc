...
namespace XBMCAddon
{
  namespace xbmcaddon
  {
     ...
     * Addon class.
     * 
     * Addon([id, installedOnly) -- Creates a new Addon class.
     * 
     * id            : [opt] string - id of the addon (default is current addon id)
     * installedOnly : [opt] boolean - False to allow uninstalled addons (default=True)
     * 
    ...
    public:
       ...
       *  - self.Addon = xbmcaddon.Addon(id='script.recentlyadded')
       */
      Addon(const char* id = NULL, bool installedOnly = true) throw (AddonException);
