...
public:
  ...
  AddonFileItemProperty(const std::string& strKey, const std::string& strValue) :
    m_strKey(strKey),
    m_type(VFS_FILE_ITEM_PROPERTY_TYPE_STRING),
    m_strValue(strValue),
    m_iValue(0),
    m_fValue(0.0)
  {
  ...
  AddonFileItemProperty(const std::string& strKey, int iValue) :
    m_strKey(strKey),
    m_type(VFS_FILE_ITEM_PROPERTY_TYPE_INT),
    m_iValue(iValue),
    m_fValue(0.0)
  {
  }
  ...
  AddonFileItemProperty(const std::string& strKey, double fValue) :
    m_strKey(strKey),
    m_type(VFS_FILE_ITEM_PROPERTY_TYPE_DOUBLE),
    m_iValue(0),
    m_fValue(fValue)
  {
  }
  ...
  AddonFileItemProperty(const CONTENT_ADDON_FILE_PROPERTY& prop) :
    m_strKey(prop.key),
    m_type(prop.type),
    m_strValue(prop.type == VFS_FILE_ITEM_PROPERTY_TYPE_STRING ? prop.strValue : ""),
    m_iValue(prop.type == VFS_FILE_ITEM_PROPERTY_TYPE_INT ? prop.iValue : 0),
  ...
  CONTENT_ADDON_FILE_PROPERTY AsProperty(void) const
  {
    ...
    prop.type = m_type;
    if (m_type == VFS_FILE_ITEM_PROPERTY_TYPE_STRING)
      prop.strValue = strdup(m_strValue.c_str());
    else if (m_type == VFS_FILE_ITEM_PROPERTY_TYPE_INT)
  ...
  {
    SAFE_FREE(prop.key);
    if (prop.type == VFS_FILE_ITEM_PROPERTY_TYPE_STRING)
...
public:
  ...
  std::string GetAndRemovePropertyString(const std::string& strKey, const std::string& strDefault = "")
  {
    ...
    std::map<std::string, AddonFileItemProperty>::iterator it = m_properties.find(strKey);
    if (it != m_properties.end() && it->second.Type() == VFS_FILE_ITEM_PROPERTY_TYPE_STRING)
    {
  ...
  int GetAndRemovePropertyInt(const std::string& strKey, int iDefault = 0)
  {
    ...
    std::map<std::string, AddonFileItemProperty>::iterator it = m_properties.find(strKey);
    if (it != m_properties.end() && it->second.Type() == VFS_FILE_ITEM_PROPERTY_TYPE_INT)
    {
